			...
			void (*idle_callback_in)(void))
{
	...
	if(use_sleep)
    {
        //Sleep initialization part. Better check and change for your project
//        set_sleep_mode(SLEEP_MODE_PWR_DOWN);
//        
//        WDTCR =
//        (1 << WDP3) |  //1024k oscillator cycles 8s
//        (0 << WDP2) |
//        (0 << WDP1) |
//        (1 << WDP0);
//        
...
//        (1 << USISIE);
	...
	for(;;)
	{
        ...
        get_voltage(); //Gets voltage continuously
        
		if(use_sleep && (ss_state == ss_state_before_start))
        {
//          Sleep codes parts.Better check and change for your project
            
//            WDTCR |= (1 << WDE);// Enable WatchDog Timer
//            WDTCR |= (1 << WDIE); // Enable WatchDog Timeout Interupt
//           
//            MCUCR |= (1 << SE);   // Sleep Enable
//            sei();
//            sleep_mode(); // Same as sleep_cpu()
//        
//            wdt_reset();
//
//            MCUCR &= ~(1 << SE);  //Clears Sleep Enable//
//            
//            WDTCR |= (1 << WDCE); // Disable WatchDog Timer
//            WDTCR |= (0 << WDE); // Disable WatchDog Timer
        }
